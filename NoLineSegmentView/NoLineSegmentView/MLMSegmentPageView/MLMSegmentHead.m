...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        button.frame = CGRectMake(start_x, (SCROLL_HEIGHT - _slideHeight)/2, width-10, _slideHeight);
        start_x += width;
        if (titles) {
            [button setTitleColor:_deSelectColor forState:UIControlStateNormal];
            [button setBackgroundColor:_deSelectBackColor];
            button.layer.cornerRadius = _slideCorner;
...
- (UIButton *)slideView {
    ...
    UIButton *slide = [[UIButton alloc] initWithFrame:CGRectMake(0, (SCROLL_HEIGHT-_slideHeight)/2, (slide_w-10)*_slideScale, _slideHeight)];
    ...
    [slide setImage:[UIImage imageNamed:@"voucher_bg_pic"] forState:UIControlStateNormal];
    ...
    slideScroll.frame = CGRectMake(0, -(SCROLL_HEIGHT - _slideHeight)/2, SCROLL_WIDTH, SCROLL_HEIGHT);
...
//    [titlesScroll setContentOffset:CGPointMake(index * sum_width, 0) animated:YES];
